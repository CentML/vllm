#!/usr/bin/env python3
"""
Functional test for v33 TMA kernel - tests correctness vs Triton reference.

The v33 kernel (fused_recurrent_gated_delta_rule_v33_tma.cu) uses:
- TMA for state tensor loading
- Standard thread block (no clusters)
- Works only for spec=0 (1 token per sequence)

This test verifies the v33 CUDA kernel produces the same outputs as the
...
# Load v33 kernel from pre-compiled .so
_lib_v33 = None
v33_available = False

try:
    lib_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'libgdr_v33_tma.so')
    _lib_v33 = ctypes.CDLL(lib_path)
    ...
    _lib_v33.launch_gdr_tma.restype = None  # Returns void
    
    v33_available = True
    print("✓ V33 TMA kernel loaded from libgdr_v33_tma.so")
except OSError as e:
    print(f"✗ V33 kernel not available: {e}")
    sys.exit(1)
...
def test_v33_vs_triton(batch_size: int, tp_size: int = 4, seed: int = 42, verbose: bool = True):
    """
    Test v33 kernel against Triton reference for spec=0 (1 token per sequence).
    ...
    o_triton, final_state_triton = fused_recurrent_gated_delta_rule_fwd(
        ...
        use_qk_l2norm_in_kernel=True,  # v33 uses L2 norm
    )
    ...
    state_tensor = initial_state.clone()  # Fresh copy for v33
    ...
    result = _lib_v33.create_tma_descriptor_state_tensor(
    ...
    o_v33_flat = o_v33.squeeze(0) if o_v33.dim() > 3 else o_v33
    
    # Compare output tensors
    o_diff = (o_v33_flat.float() - o_triton_flat.float()).abs()
    ...
    ht_v33_subset = ht_v33[:batch_size]
    
    ht_diff = (ht_v33_subset.float() - ht_triton_subset.float()).abs()
    ...
    if verbose:
        ...
        print(f"      V33 o[0,0,:5]:    {o_v33_flat[0, 0, :5].tolist()}")
    
    # Compute exact match rates
    exact_match_o = (o_triton_flat == o_v33_flat).float().mean().item() * 100
    exact_match_ht = (ht_triton_subset == ht_v33_subset).float().mean().item() * 100
    ...
    del o_triton, final_state_triton, o_v33, ht_v33
    del o_diff, o_flat, ht_diff, ht_flat, tma_desc_device
    del o_triton_flat, o_v33_flat, ht_triton_subset, ht_v33_subset
...
def run_all_tests():
    ...
    print("=" * 80)
    print("V33 TMA Kernel Correctness Test vs Triton Reference")
    print("=" * 80)
    ...
    for batch_size, tp_size in valid_configs:
        try:
            ...
            result = test_v33_vs_triton(batch_size=batch_size, tp_size=tp_size, verbose=False)
