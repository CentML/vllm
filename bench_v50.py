...
Compares Triton (all specs) vs CUDA v33 TMA (PTX asm) vs CUDA v50 TMA (CUTLASS) for spec=0
...
# Load v33 TMA kernel (PTX assembly barriers)
...
if __name__ == "__main__":
    ...
    print(f"Comparing: Triton (baseline) vs v33 (PTX asm barriers) vs v50 (CUTLASS barriers)")
    print(f"{'='*110}")
    
    # Run benchmarks
    results = {}
    total = len(TP_SIZES) * len(BATCH_SIZES) * (3 + 3)  # 3 kernels for spec0, 3 specs for triton
    current = 0
    ...
    for tp in TP_SIZES:
        ...
        for bs in BATCH_SIZES:
            results[tp][bs] = {}
            # Spec=0: all three kernels
            for kernel in ['triton', 'cuda_v33', 'cuda_v50']:
                current += 1
    ...
    for tp in TP_SIZES:
        print(f"\nTP={tp}")
        print(f"{'Batch':>6} | {'-------SPEC=0-------':^48} | {'SPEC=1':>8} | {'SPEC=2':>8} | {'SPEC=3':>8}")
        print(f"{'':>6} | {'Triton':>8} {'v33(asm)':>10} {'v50(CUT)':>10} {'v33 spd':>8} {'v50 spd':>8} | {'Triton':>8} | {'Triton':>8} | {'Triton':>8}")
        print("-" * 110)
        
        for bs in BATCH_SIZES:
            r = results[tp][bs]
            speedup_v33 = r['s0_triton'] / r['s0_cuda_v33']
            speedup_v50 = r['s0_triton'] / r['s0_cuda_v50']
            print(f"{bs:>6} | {r['s0_triton']:>8.1f} {r['s0_cuda_v33']:>10.1f} {r['s0_cuda_v50']:>10.1f} "
                  f"{speedup_v33:>7.2f}x {speedup_v50:>7.2f}x |"
                  f" {r['s1_triton']:>8.1f} | {r['s2_triton']:>8.1f} | {r['s3_triton']:>8.1f}")
    
    # Summary
    speedups_v33 = [results[tp][bs]['s0_triton'] / results[tp][bs]['s0_cuda_v33'] 
                    for tp in TP_SIZES for bs in BATCH_SIZES]
    speedups_v50 = [results[tp][bs]['s0_triton'] / results[tp][bs]['s0_cuda_v50'] 
                    for tp in TP_SIZES for bs in BATCH_SIZES]
    speedups_v33 = np.array(speedups_v33)
    speedups_v50 = np.array(speedups_v50)
    
    print(f"\n{'='*110}")
    print(f"SUMMARY (Spec=0):")
    print(f"  v33 (PTX asm):       Min={speedups_v33.min():.2f}x  Max={speedups_v33.max():.2f}x  Mean={speedups_v33.mean():.2f}x")
    print(f"  v50 (CUTLASS):       Min={speedups_v50.min():.2f}x  Max={speedups_v50.max():.2f}x  Mean={speedups_v50.mean():.2f}x")
    
    # Compare v50 vs v33 directly
    ratios = speedups_v50 / speedups_v33
    pct_diff = (ratios - 1.0) * 100
    
    print(f"\n  v50 vs v33:          Min={ratios.min():.4f}  Max={ratios.max():.4f}  Mean={ratios.mean():.4f}")
    if ratios.mean() > 1.0:
        print(f"  → v50 is {pct_diff.mean():.2f}% faster than v33 on average (CUTLASS barriers win!)")
    elif ratios.mean() < 1.0:
        print(f"  → v33 is {-pct_diff.mean():.2f}% faster than v50 on average (PTX asm barriers win)")
    else:
        print(f"  → v50 and v33 have identical performance")
