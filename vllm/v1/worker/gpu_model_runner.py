...
class GPUModelRunner(
        ...
        try:
            with DeviceMemoryProfiler() as m:
                ...
                if self.use_aux_hidden_state_outputs:
                    ...
                    aux_layers = (
                        self._get_dflash_aux_layers_from_config()
    ...
    def _get_dflash_aux_layers_from_config(self) -> tuple[int, ...] | None:
        """Extract D-Flash auxiliary layer indices from speculative config.

        These indices specify which hidden states from the base model should
        be used as auxiliary inputs for the D-Flash drafter model during
        speculative decoding.
        ...
        hf_config = self.speculative_config.draft_model_config.hf_config
        dflash_config = getattr(hf_config, "dflash_config", None)
        if dflash_config is None or not isinstance(dflash_config, dict):
            return None
