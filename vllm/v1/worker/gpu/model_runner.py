...
class GPUModelRunner(LoRAModelRunnerMixin):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        self._idx_mapping_staging = PooledPinnedBuffer(self.max_num_reqs, torch.int64)
        self._cu_num_logits_staging = PooledPinnedBuffer(
            self.max_num_reqs + 1, torch.int32
        )
        self._query_start_loc_staging = PooledPinnedBuffer(
            self.max_num_reqs + 1, torch.int32
        )
    ...
    def prepare_inputs(
        ...
        idx_mapping = self._idx_mapping_staging.copy_to_gpu(
            idx_mapping_np, device=self.device
        )
